...
const toTitleCase = (arr) => {
  let output = [];
  arr.forEach((str, idx) => {
    output.push(str.toUpperCase()[0] + str.slice(1));
  });
  return output;
};
...
const detectTicTacToeWin = (board) => {
  for (let index = 0; index < board.length; index++) {
    if (isEqual(board[index][0] + board[index][1] + board[index][2])) {
      return true;
    } else if (isEqual(board[0][index] + board[1][index] + board[2][index])) {
      return true;
    } else if (isEqual(board[0][0] + board[1][1] + board[2][2])) {
      return true;
    } else if (isEqual(board[0][1] + board[1][1] + board[2][2])) {
      return true;
    }
    return false;
  }
