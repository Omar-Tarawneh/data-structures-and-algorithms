...
describe("Testing Repeated words function", () => {
  ...
  test("happy path should return first repeated word in the str", () => {
    expect(repeatedWord(str1)).toEqual("a");
    expect(repeatedWord(str2)).toEqual("it");
    expect(repeatedWord(str3)).toEqual("summer");
  });
  test("Faliure if the str is empty return null", () => {
    expect(repeatedWord("")).toBeNull();
  });
  test("if the str contain only one word", () => {
    expect(repeatedWord("Omar?")).toBeNull();
  });
});
